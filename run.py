...
def train():
    ...
    if cfg.mode in['FREE', 'PGD', "FGSM"]:
        ...
        #delta = torch.zeros(cfg.batch_size, cfg.seq_length, cfg.embedding_size).cuda()
        delta.requires_grad = True

    #parameters in paper
    epsilon = torch.tensor(0.1).cuda()
    alpha= 0.04
    attack_iters=5
    ...
    for epoch in range(1, 1 + cfg.epochs):
        ...
        for step, batch in enumerate(data_manager):
            ...
            if cfg.mode=="FGSM":
                ...
                delta.data[:input_ids[0].size(0)] = clamp(delta[:input_ids[0].size(0)], -epsilon, epsilon)  # 防止image越界
...
if __name__ == '__main__':
    ...
    cfg=TextConfig()
    cfg.mode = sys.argv[2] # FGSM PGD FREE default
    cfg.save_path='./save_models/'+cfg.mode+'/best_model.weights'
    cfg.history_dir='./save_models/'+cfg.mode+'/history.txt'
